...
pub fn hex_to_int(hex_str: &str) -> u16 {
...
impl Emulator {
    ...
    // fn fetch_sprite_addr(&self, digit: u8) -> [u8; 5] {
    //     let mut sprite: [u8; 5] = [0;5];
    //     let start: usize = (digit * 5) as usize;
    //     for idx in 0..sprite.len() {
    //         sprite[idx] = self.memory[start + idx];
    //     }
    ...
    pub fn read_rom(&mut self, program: &mut Vec<u8>) {
        ...
        for idx in 0..self.program_size {
            self.memory[addr + idx] = program[idx];
        }
    }
    ...
    pub fn run(&mut self) {
        ...
        for idx in (0..(self.program_size - 1)).step_by(2) {
            let b1: u8 = self.memory[self.PROGRAM_START + idx];
            let b2: u8 = self.memory[self.PROGRAM_START + idx+1];
        ...
        while self.running {
            ...
            let opcode = self.convert_to_opcode(self.memory[self.cpu.PC as usize], self.memory[(self.cpu.PC + 1) as usize]);
                ...
                match &opcode_str[2..] {
                    ...
                    "33" => {
                        ...
                        for idx in 0..3 {
                            ...
                            self.memory[(vi + 2 - idx) as usize] = digit;
                    ...
                    "55" => {
                        ...
                        for idx in 0..vx_addr {
                            self.memory[(vi as usize + idx)] = self.cpu.registers[idx];
                        }
                    },
                    "65" => {
                        // LD Vx, [I]
                        ...
                        for idx in 0..(vx_addr+1) {
                            self.cpu.registers[idx] = self.memory[vi as usize + idx];
                        }
                    },
                    _ => {
                        // Error
